...
class  OdfObjectImpl(
  ...
  def combine(another: OdfObject): OdfObject = {
    ...
    OdfObject(
      ids,
        //case Seq(OdfQlmIDType(valueA, attributesA), OdfQlmIDType(valueB, attributesB)) => //idTypeB, tagTypeB, startDateB, endDateB, attrB)) =>
          //OdfQlmIDType(valueB, attributesA ++ attributesB)},//,
