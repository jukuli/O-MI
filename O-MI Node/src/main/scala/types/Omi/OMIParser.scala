...
object OMIParser extends parsing.Parser[OmiParseResult] {
  ...
  private[this] def parseMsg(msg: xmlGen.xmlTypes.MsgType, format:Option[String]): odf.OdfParseResult ={
  ...
  private[this] def parseOdf(node: Node): odf.OdfParseResult = odf.ODFParser.parse(node)
