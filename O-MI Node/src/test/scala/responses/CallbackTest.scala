...
import types.omi.{RawCallback, ReadRequest, UserInfo}
...
Request with callback
  ...
  def ct1 = {
    ...
    dummy.defineCallbackForRequest(ReadRequest(ImmutableODF(), callback = Some(differentCb), user0 = UserInfo(Some(RemoteAddress(googleIP)))), None)
  ...
  def ct2 = {
    ...
    dummy.defineCallbackForRequest(ReadRequest(ImmutableODF(), callback = Some(cb), user0 = UserInfo(Some(RemoteAddress(googleIP)))),None)
